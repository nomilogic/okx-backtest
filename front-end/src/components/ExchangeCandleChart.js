...
const ExchangeCandleChart = ({ data }) => {
    ...
    useEffect(() => {
        ...
        const numberLength = (num) => num.toString().replace('.', '').replace('-', '').length;
        if (data) {
            let numberLength=0;
            if(data[0] && data[0].open<1)
           { 
            numberLength=1+"0".repeat(data[0].open.toExponential().split(".")[1].split("e-").reduce((a,b)=>{ return a.length+Number(b)}))
