...
class CryptoOrder {
    ...
    fill(currentPrice = this.price) {
        ...
        this.status = "filled";
        console.log(
            `${this.type.charAt(0).toUpperCase() + this.type.slice(1)} order ${this.orderId
...
class AdvancedOrder {
    ...
    constructor({
        ...
        if (this.position === "long") {
            this.createNewBuyLimitOrder(currentPrice)
        }
        else if (this.position === "short") {
            this.createNewSellLimitOrder(currentPrice)
        }
    ...
    checkAndCreateSell(currentPrice) {
        const checkbuy =
            this.buyOrder !== null
                ? this.checkAndPerfomOrderBuyOrSell(this.buyOrder, currentPrice)
                : "";
        const checkSell =
            null !== this.sellOrder
                ? this.checkAndPerfomOrderBuyOrSell(this.sellOrder, currentPrice)
                : "";
        ...
        if (this.position === "short") {
            if (this.sellOrder.filled && !this.buyOrder) {

                let orderInfo = {
                    type: "buy",
                    price: this.buyPrice,
                    quantity: this.buyQuantity,
                    feeRate: this.buyerFeeRate,
                };
                this.buyOrder = this.orderManager.CreateNewOrder(
                    orderInfo.type,
                    orderInfo.price,
                    orderInfo.quantity
                );
            }
            if (this.buyOrder && this.buyOrder.filled) {
                // this.buyOrder.fill(currentPrice);
                this.checkPnl();
                console.log(`Advanced order completed with PnL: ${this.pnl}`);
                this.orderManager.addOrderBnS({
                    buyOrder: this.buyOrder,
    ...
    checkPnl() {
        if (this.position == "long") {
            this.pnl = Number(
                (
                    this.sellOrder.filledAt * this.sellOrder.quantity -
                    this.sellOrder.fee -
                    this.buyOrder.filledAt * this.buyOrder.quantity
                ).toFixed(8)
            )
        }
        else if (this.position == "short") {
    ...
    checkAndPerfomOrderBuyOrSell(order, currentPrice) {
        if (order.type === "buy" && !order.filled && currentPrice <= order.price) {
            ...
            order.fill(currentPrice);
            console.log(order, currentPrice);
        }
    ...
    createNewSellLimitOrder(currentPrice) {
        let orderInfo = {
            type: "sell",
            price: this.sellPrice,
        ...
        this.buyOrder = null;
        this.buyQuantity = Utils.toFixedNoRound(this.quantity - this.sellOrder.fee, 2)

        console.log(
            `New sell order created at price: ${currentPrice} to sell at ${this.sellPrice}`
        );
        console.log(this.sellOrder);
    }

    createNewBuyLimitOrder(currentPrice) {
        let orderInfo = {
            type: "buy",
            price: this.buyPrice,
            quantity: this.quantity,
...
class Grid {
    ...
    formatElapsedTime(ms) {
        ...
        return days > 0
            ? `${days}d ${hours}:${minutes.toString().padStart(2, "0")}:${seconds
                .toString()
                .padStart(2, "0")}`
    ...
    updatePnL(currentPrice, position = this.position) {
        if (position === 'short') {
            // Calculate PnL for short orders
            this.totalPnL = this.orderManager.ordersBnS.reduce((total, orderBnS) => {
                if (orderBnS.sellOrder.filled) {
                    return total + (orderBnS.sellOrder.filledAt - orderBnS.buyOrder.filledAt);
                }
                return total;
            }, 0);
        } else if (position === this.position) {
            // Calculate PnL for long orders
            this.totalPnL = this.orderManager.ordersBnS.reduce((total, orderBnS) => {
                if (orderBnS.buyOrder.filled) {
                    return total + (orderBnS.buyOrder.filledAt - orderBnS.sellOrder.filledAt);
                }
                return total;
            }, 0);
        }
    ...
    usdtUsed(position = "long") {
        if (position === 'short') {
            // Calculate USDT used for short orders
            return this.activeOrders.reduce((total, order) => {
                if (order.sellOrder.filled && !order.buyOrder.filled) {
                    return total + order.quantity * order.sellOrder.filledAt;
                }
                return total;
            }, 0);
        } else if (position === 'long') {
            // Calculate USDT used for long orders
            return this.activeOrders.reduce((total, order) => {
                if (order.buyOrder.filled && !order.sellOrder.filled) {
                    return total + order.quantity * order.buyOrder.filledAt;
                }
                return total;
            }, 0);
        }
    ...
    getGridSummary(currentPrice) {
        ...
        const currentHoldings = {
            crypto: this.activeOrders.reduce(
                (total, order) => total + (order.buyOrder.filled ? order.quantity : 0),
                0
            ),
        ...
        return {
            ...
            totalPnL: {
                ...
                percentage: `${((this.totalPnL / this.investmentAmount) * 100).toFixed(
                    2
                )}%`,
            ...
            executionOverview: {
                totalExecutions: this.orderManager.orders.filter(
                    (order) => order.filled
                ).length,
                ...
                gridOrderDetails: {
                    ...
                    buyOrders: this.orderManager.getOrdersByStatus("open", "buy")
                        .buyOrders.length,
                    sellOrders: this.orderManager.getOrdersByStatus("open", "sell")
                        .sellOrders.length,
                },
            },
    ...
    updatePrice(currentPrice) {
        ...
        this.currentPrice = this.lastPrice = currentPrice;
    }
...
class PriceManager extends EventDispatcher {
    ...
    updatePrice() {
        const percentChange =
            ...
            100;
        this.price = parseFloat((this.price * (1 + percentChange)).toFixed(8)); // Keep two decimal points
    ...
    start(interval = 1000, targetTime = "1m", speedMultiplier = 1) {
        ...
        this.priceLoop = setInterval(() => {
            ...
            if (this.elapsedMs >= targetDuration) {
                ...
                this.dispatchEvent(PriceManager.Events.COMPLETED, this.onCompelete()); // Dispatch completed event
    ...
    startFast(
        ...
        const processBatch = () => {
            ...
            console.log(
                `Batch complete ${batchs}, ${this.price} ,${new Date(
                    PriceManager.time
                ).toDateString()}`
            );
...
priceManager.addEventListener(PriceManager.Events.COMPLETED, (result) => {
    console.log(
        `Price fluctuation completed. Final price: $${result.finalPrice.toFixed(
            8
        )}. Elapsed time: ${result.elapsedTime}. Message: ${result.message}`
    );
